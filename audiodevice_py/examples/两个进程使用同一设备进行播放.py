...
# 直接改这里即可，无需命令行参数
# 两个进程会并发播放到同一个设备（same_play_device）
TEST_CONFIG: dict[str, Any] = {
    "same_play_device": (10, 12),
    "proc_1": {
...
def print_report(results: list[dict[str, Any]]) -> None:
    ...
    if success == 2:
        print("两个进程都成功：同一设备并发播放可用或被后端混音处理。")
    elif success == 1:
        print("一个成功一个失败：同设备并发播放存在占用冲突迹象。")
    elif success == 0:
...
def run() -> None:
    same_device = TEST_CONFIG["same_play_device"]
    cfg1 = {
        "device": same_device,
