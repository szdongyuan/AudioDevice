...
TEST_CONFIG: dict[str, Any] = {
    "proc_1": {
        ...
        "channels": 1,
        "device": (14, 17),
        "save_dir": "recordings/two_proc_two_devices/proc1",
    },
    "proc_2": {
        "samplerate": 48000,
        "seconds": 8.0,
        "channels": 6,
        "device": (15, 18),
        "save_dir": "recordings/two_proc_two_devices/proc2",
    },
    "start_gap_ms": 5.0,
    "result_timeout": 20.0,
}
...
def print_report(results: list[dict[str, Any]]) -> None:
    ...
    if success == 2:
        print("两个进程都成功：两台设备并行录音正常。")
    elif success == 1:
        print("一个成功一个失败：某一台设备配置、权限或占用状态异常。")
    elif success == 0:
...
def run() -> None:
    cfg1 = {
        "samplerate": int(TEST_CONFIG["proc_1"]["samplerate"]),
        "seconds": float(TEST_CONFIG["proc_1"]["seconds"]),
        "channels": int(TEST_CONFIG["proc_1"]["channels"]),
        "device": TEST_CONFIG["proc_1"]["device"],
        "save_dir": str(TEST_CONFIG["proc_1"]["save_dir"]),
    }
    cfg2 = {
        "samplerate": int(TEST_CONFIG["proc_2"]["samplerate"]),
        "seconds": float(TEST_CONFIG["proc_2"]["seconds"]),
        "channels": int(TEST_CONFIG["proc_2"]["channels"]),
        "device": TEST_CONFIG["proc_2"]["device"],
        "save_dir": str(TEST_CONFIG["proc_2"]["save_dir"]),
    }
    ...
    print(
        ...
        f"seconds={cfg1['seconds']}, channels={cfg1['channels']}, dtype=float32, "
        f"save_dir={cfg1['save_dir']})"
    )
    print(
        f"proc-2(device={cfg2['device']}, samplerate={cfg2['samplerate']}, "
        f"seconds={cfg2['seconds']}, channels={cfg2['channels']}, dtype=float32, "
        f"save_dir={cfg2['save_dir']})"
    )
    print(f"start_gap_ms={start_gap_ms}, result_timeout={result_timeout}")

    result_q: mp.Queue = mp.Queue()
    p1 = mp.Process(target=record_worker, args=("proc-1", cfg1, result_q), daemon=False)
    p2 = mp.Process(target=record_worker, args=("proc-2", cfg2, result_q), daemon=False)
    ...
    max_seconds = max(cfg1["seconds"], cfg2["seconds"])
    deadline = time.perf_counter() + max(result_timeout, max_seconds + 5.0)
