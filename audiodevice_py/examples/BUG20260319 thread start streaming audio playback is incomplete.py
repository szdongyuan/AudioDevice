...
def _thread_worker(
    *,
    ...
    try:
        ...
        if lib_name == "audiodevice":
            ...
            def callback(indata, outdata, frames, time_info, status) -> None:  # noqa: ARG001
                ...
                if remain <= 0:
                    outdata.fill(0.0)
                    done_event.set()
                    print("callback time:", time.perf_counter() - t001)
                    raise callback_stop_exc()
        ...
        try:
            wait_deadline = time.perf_counter() + (total_frames / float(SAMPLERATE)) + 5.0
            while not done_event.is_set() and time.perf_counter() < wait_deadline:
                time.sleep(0.01)
            ...
            print(
                f"排空时长={t2-t3}, 喂入耗时={t3-t4}, 播放总时长={t2-t4}, 启动时长={t4-t1}, 总时长={t2-t1}"
            )
